...
class FactorizedAttacker:
    ...
    def _init_factor_library(self):
        """
        初始化触发器因子库（增强版 - 支持多种类型）
        创建所有可用的因子实例，增加触发器隐蔽性
        """
        factors = []

        # 从配置读取是否启用各类因子
        enable_position = self.config.get('trigger_factorization', {}).get('enable_position', True)
        enable_frequency = self.config.get('trigger_factorization', {}).get('enable_frequency', True)
        enable_color = self.config.get('trigger_factorization', {}).get('enable_color', True)
        enable_geometric = self.config.get('trigger_factorization', {}).get('enable_geometric', True)

        # 1. 位置扰动因子（如果启用）
        if enable_position:
            pos_config = self.config.get('trigger_factorization', {}).get('position_perturbation', {})
            intensity = pos_config.get('intensity', 0.12)
            ...
            for pos, pattern, desc in positions:
                factors.append(PositionPerturbationFactor(
                    position=pos,
                    size=pos_config.get('size', 4),
                    pattern=pattern,
                    intensity=intensity
                ))
            print(f"  ✓ 添加 {len(positions)} 个位置扰动因子")

        # 2. 频域扰动因子（新增 - 高隐蔽性）
        if enable_frequency:
            freq_config = self.config.get('trigger_factorization', {}).get('frequency_perturbation', {})
            intensity = freq_config.get('intensity', 0.03)
            ...
            print(f"  ✓ 添加 3 个频域扰动因子 (隐蔽性强)")

        # 3. 颜色偏移因子（新增 - 高隐蔽性）
        if enable_color:
            color_config = self.config.get('trigger_factorization', {}).get('color_shift', {})
            intensity = color_config.get('intensity', 0.08)
            ...
            print(f"  ✓ 添加 3 个颜色变换因子 (隐蔽性强)")

        # 4. 几何扰动因子（新增 - 中等隐蔽性）
        if enable_geometric:
            geom_config = self.config.get('trigger_factorization', {}).get('geometric_perturbation', {})
            intensity = geom_config.get('intensity', 0.08)
            ...
            print(f"  ✓ 添加 3 个几何变换因子")

        print(f"  📊 因子库总计: {len(factors)} 个因子")
        print(f"     - 位置扰动: {5 if enable_position else 0}")
        print(f"     - 频域扰动: {3 if enable_frequency else 0}")
        print(f"     - 颜色变换: {3 if enable_color else 0}")
        print(f"     - 几何变换: {3 if enable_geometric else 0}")
