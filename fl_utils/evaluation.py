...
class FactorizedAttackEvaluator:
    ...
    def evaluate_main_task(self, model, epoch=None):
        """评估主任务准确率"""
        model.eval()
        correct = 0
        total = 0
    ...
    def evaluate_attack_success_rate(self, model, adversary_id, epoch):
        ...
        poison_start = getattr(self.config, 'poison_start_epoch', 0)
        if epoch is not None and epoch < poison_start:
            # 投毒尚未开始，避免ASR出现虚高
            return 0.0
    ...
    def calculate_stealthiness(self, local_model, global_model):
        ...
        total_norm = 0.0
        with torch.no_grad():
            for (name_local, param_local), (name_global, param_global) in zip(
                local_model.named_parameters(),
                global_model.named_parameters()
            ):
                if name_local != name_global:
                    continue
                diff = param_local.detach() - param_global.detach()
                total_norm += torch.sum(diff * diff).item()
    ...
    def comprehensive_evaluation(self, model, epoch, log_history=True):
        ...
        # 1. 主任务准确率
        print(f"\n[1/7] 评估主任务准确率...")
        main_acc = self.evaluate_main_task(model, epoch)
