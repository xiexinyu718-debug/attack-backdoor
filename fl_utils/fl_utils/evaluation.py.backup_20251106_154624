...
class FactorizedAttackEvaluator:
    ...
    def comprehensive_evaluation(self, model, epoch):
        ...
        # 4. 隐蔽性
        print(f"\n[4/6] 评估隐蔽性...")
        if hasattr(self.helper, 'global_model'):
            stealth_scores = []
            num_adversaries = self.config.get('num_adversaries', 1)
            for adv_id in range(num_adversaries):
                if adv_id < len(self.helper.adversary_list):
                    adv_participant_id = self.helper.adversary_list[adv_id]
                    if adv_participant_id < len(self.helper.client_models):
                        local_model = self.helper.client_models[adv_participant_id]
                        stealth = self.calculate_stealthiness(
            ...
            print(f"  平均隐蔽性: {results['stealthiness']:.4f}")
        else:
            results['stealthiness'] = None
            print(f"  隐蔽性: N/A (无全局模型)")
