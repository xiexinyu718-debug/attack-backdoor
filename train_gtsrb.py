"""
GTSRB数据集训练脚本 - 使用现有框架
完全兼容train_baseline_with_stealth.py的架构
"""
import os
...
def train_benign_client(helper, participant_id, model, epoch):
    ...
    return model, {'avg_loss': avg_loss, 'accuracy': accuracy, 'total_samples': total_samples}
...
def train_malicious_client(helper, participant_id, model, epoch, attacker, trainer):
    ...
    final_stats = {
        ...
        'poisoned_samples': all_stats[-1]['poisoned_samples'],  # 修复：只取最后一次
...
def federated_learning_round(helper, attacker, trainer, epoch):
    ...
    # 【关键】初始化client_models列表（用于隐蔽性评估）
    helper.client_models = []
    local_models = {}
    training_stats = {}

    for idx, participant_id in enumerate(sampled_participants):
        progress = f"[{idx+1}/{len(sampled_participants)}]"
        print(f"{progress} 处理参与者 {participant_id}...")
        ...
        if participant_id in helper.adversary_list:
            ...
            print(f"  训练良性客户端 {participant_id}...")
            local_model, stats = train_benign_client(
                helper, participant_id, local_model, epoch
            )
            print(f"  ✓ 完成: Loss={stats['avg_loss']:.4f}, Acc={stats['accuracy']:.2f}%")
...
def main():
    parser = argparse.ArgumentParser(description='GTSRB后门攻击训练（完整框架）')
    ...
    report_path = f"{helper.folder_path}/gtsrb_report.json"
