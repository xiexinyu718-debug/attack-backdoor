...
def aggregate_models(helper, local_models):
    ...
    # 初始化权重累加器
    weight_accumulator = {}
    for name, param in helper.global_model.state_dict().items():
        weight_accumulator[name] = torch.zeros_like(param)
    
    # 累加所有本地模型的更新
    for participant_id, local_model in local_models.items():
        for name, param in local_model.state_dict().items():
            weight_accumulator[name] += (param - helper.global_model.state_dict()[name])
    
    # 平均并更新全局模型
    num_participants = len(local_models)
    for name, param in helper.global_model.state_dict().items():
        update = weight_accumulator[name] / num_participants
        param.add_(update)
...
def main():
    ...
    for epoch in range(helper.config.epochs):
        ...
        # 保存模型
        if epoch % helper.config.save_every == 0:
